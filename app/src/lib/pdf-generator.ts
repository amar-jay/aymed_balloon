...
export async function generateSessionPDF(options: PDFOptions): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Table Configuration
    const colWidths = [20, 60, 60, 60, 60, 80, 180] // Total should be ~495
    const columns = [
      { header: '#', align: 'left' },
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      // Temporarily disable bottom margin to prevent auto-page creation
      ...
      doc
        ...
        .text(
          ...
          doc.page.height - 30,
          {
            width: THEME.layout.contentWidth - 60,
            align: 'left',
            lineBreak: false
          }
        )
      ...
      doc.text(
        ...
        doc.page.height - 30,
        { align: 'right', width: 50, lineBreak: false }
      )
